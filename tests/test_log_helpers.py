...
class TestPrettyConsoleFormatterUpstreamRequest:
    def test_basic_upstream_request(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
            event=LogEvent.UPSTREAM_REQUEST.value,
            message="PROXY->Provider POST model-x",
    ...
    def test_upstream_request_sync(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
            event=LogEvent.UPSTREAM_REQUEST.value,
            message="PROXY->Provider POST model-y",
            data={
                "target_model": "model-y",
                "stream": False,
                "last_user_prompt": "test prompt",
            },
        )
        py_record = _make_python_log_record(lr)
        result = formatter.format(py_record)
    ...
    def test_upstream_request_no_prompt(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
            event=LogEvent.UPSTREAM_REQUEST.value,
            message="PROXY->Provider POST model-z",
            data={
        ...
        assert "PROXY→Provider" in result
        assert "⚡stream" in result
        # No prompt display when last_user_prompt is missing
        assert '"' not in result

    def test_upstream_request_with_request_id(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
            event=LogEvent.UPSTREAM_REQUEST.value,
            message="PROXY->Provider POST model",
            request_id="abcdefgh12345678",
            data={"target_model": "model", "stream": False},
        )
        py_record = _make_python_log_record(lr)
        result = formatter.format(py_record)
        # request_id[:8] = "abcdefgh"
        assert "abcdefgh" in result


class TestPrettyConsoleFormatterUpstreamResponse:
    def test_sse_stream_response(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
            event=LogEvent.UPSTREAM_RESPONSE.value,
            message="Provider->PROXY 200",
            data={
                "status_code": 200,
                "body_type": "sse_stream",
            },
        )
        py_record = _make_python_log_record(lr)
        result = formatter.format(py_record)
        ...
        assert "<SSE stream>" in result

    def test_json_response_with_tokens(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
            event=LogEvent.UPSTREAM_RESPONSE.value,
            message="Provider->PROXY 200",
        ...
        assert "in=150" in result
        assert "out=42" in result


class TestPrettyConsoleFormatterAnthropicRequest:
    def test_basic_anthropic_request(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
    ...
    def test_anthropic_request_with_request_id(self):
        formatter = PrettyConsoleFormatter()
        lr = _make_log_record(
            event=LogEvent.ANTHROPIC_REQUEST.value,
            message="Received Anthropic request body",
            request_id="reqid1234567890",
            data={
        ...
        # request_id[:8] = "reqid123"
        assert "reqid123" in result
